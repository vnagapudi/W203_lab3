...
3. Probability values are greater than one in some cases for probability of arrest, and probability of conviciton. Probability of prison time does not exhibit this behavior. 
...
If we assume the probability of arrest is the number of arrests or number of convictions dividied by the number of offenses, it is plausible that a given offense was committed by more than one individuak. In these cases, there could be more than one arrest or conviction for a single offense. 

The variable, then, overestimates the probability of being arrested or convicted for a given offense. This issue could be present on all observations, not just on the ones where the justice system secured enough arrests or convictions to have the varable in question be greater than one. 
...
The crmrte is negatively correlated with prbarr and prbconv_fix, which is intuitive. There is perhaps a positive correlation to prbpris, the probability of prison sentencing, which is not intuitive, but the direction of the correlation is not clear from the dataset, therefore we excluded this from our key variable set.

Looking at prbarr and prbconv_fix since we see an outlier
```{r}
plot(prbarr ~ prbconv_fix, data = crime_data_clean)
```
We see an outlier that is fairly off for prbconv = 1.5 and prb_arr = 1.1. We believe this is likely a spurious observation and will remove it from the dataset. We also found that keeping this value caused the Cook's distance to be off significantly. This is observation 51 which we will remove below:
...
For the rest of the analysis, we use crime_data_clean2.

Analyzing the average sentence ("avgsen")":
```{r}
summary(crime_data_clean2$avgsen)
cor(crime_data_clean2$crmrte,crime_data_clean2$avgsen )
```
...
```{r fig.height=4, fig.width=5, fig.align='center'}
scatterplotMatrix(~ crmrte + avgsen, data=crime_data_clean2)
```
...
scatterplotMatrix(~ crmrte + polpc +  density + taxpc, data=crime_data_clean2)
```
...
```{r}
summary(crime_data_clean2$density)
cor(crime_data_clean2$crmrte,crime_data_clean2$density)
```
...
```{r}
summary(crime_data_clean2$taxpc)
cor(crime_data_clean2$crmrte, crime_data_clean2$taxpc)
```
...
```{r}
hist(crime_data_clean2$taxpc, breaks = 50,
...
```{R}
corr_w = cor(crime_data_clean2[, c(3,4,26,6,7,8,9,10,14,25,15,16,17,18,19,20,21,22,23)])
...
```{r fig.height=4, fig.width=5, fig.align='center'}
scatterplotMatrix(~ crmrte + pctymle +  pctmin80, data=crime_data_clean2)
```
...
```{r}
summary(crime_data_clean2$pctymle)
cor(crime_data_clean2$crmrte,crime_data_clean2$pctymle)
summary(crime_data_clean2$pctmin80)
cor(crime_data_clean2$crmrte,crime_data_clean2$pctmin80)
```
...
hist(crime_data_clean2$crmrte, breaks = 30,
     main = 'Histogram of Crime Rate',
     xlab = 'Crime Rate', cex.main = 0.9, cex.lab = 0.9, cex.axis = 0.9)

crime_data_clean2['log_crmrte'] = log(crime_data_clean2$crmrte)
hist(crime_data_clean2$log_crmrte, breaks = 30,
...
```{r fig.width=3, fig.show='hold', fig.align='center'}
hist(crime_data_clean2$density, breaks = 30,
     main = 'Histogram of Density',
     xlab = 'Density', cex.main = 0.9, cex.lab = 0.9, cex.axis = 0.9)

hist(log(crime_data_clean2$density), breaks = 30,
     main = 'Histogram of Density',
     xlab = 'Density', cex.main = 0.9, cex.lab = 0.9, cex.axis = 0.9)
```
...
```{R  fig.height=4, fig.width=5, fig.align='center'}
scatterplotMatrix(~ log_crmrte + density + log(density), data = crime_data_clean2)
cor(crime_data_clean2$log_crmrte,crime_data_clean2$density)
cor(crime_data_clean2$log_crmrte,log(crime_data_clean2$density))

crime_data_clean2['log_density'] = log(crime_data_clean2$density)
```
...
scatterplotMatrix(~ log_crmrte + log(polpc), data = crime_data_clean2)
cor(crime_data_clean2$log_crmrte,crime_data_clean2$polpc)
cor(crime_data_clean2$log_crmrte,log(crime_data_clean2$polpc))

crime_data_clean2['log_polpc'] = log(crime_data_clean2$polpc)
```
...
model3_no_polpc = lm(crime_data_clean2$log_crmrte ~ 
              crime_data_clean2$prbarr + 
              crime_data_clean2$prbconv_fix + 
              crime_data_clean2$prbpris + 
              crime_data_clean2$avgsen + 
...
We can now compare all three models. We used the "model3_no_polpc" so that we can do an apples to apples comparison with model2 only with variables that can help predict crime rate. As we mentioned earlier, there is enough justification to remove polpc from our models since it is a lagging indicator.
...
Wages              | The more affluent neighborhoods will tend to have lesser crime. We thought this would be reflectedby tax revenues per capita, but it does not appear to be the case. This may be due to the nature of the taxation system itself in creating a representative variable.
