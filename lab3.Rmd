...
Now there are only 81 observerations which is less than 100. So we do need to be careful our assumptions around CLM for coefficients being normal.
...
```{r}
summary(crime_data_corr2$crmrte)
```

Looking at the histogram to look at the distribution of this key variable:
```{r}
hist(crime_data_corr2$crmrte, breaks = 50)
```

There is defintely a positive skew here. As with most rates, it might be useful to look at the log to see if it is a bit more normal.

```{r}
hist(log(crime_data_corr2$crmrte), breaks = 50)
```

This is definitely looking more normal than the original variable.
...
```{r}
scatterplotMatrix(~ log(crime_data_corr2$crmrte) + crime_data_corr2$prbarr +  crime_data_corr2$prbconv_fix + crime_data_corr2$prbpris)
```
...
```{r}
scatterplotMatrix(~ log(crime_data_corr2$crmrte) + crime_data_corr2$polpc +  crime_data_corr2$density + crime_data_corr2$taxpc)
```
Crime Rate seems to be positively correlated to the "Police per capita". This is somehow not intuitive. Perhaps because the Crime Rate is high, there are more Police needed? It is somewhat doubtful that Crime Rate is high because the Police Per Capita is high. There is likely an inverse causal relationship here.
...
```{r}
summary(crime_data_corr2$density)
cor(crime_data_corr2$crmrte,crime_data_corr2$density)
```
```{r}
hist(crime_data_corr2$density, breaks = 50)
```

The Taxpc is a proxy for how rich a county is. It is likely that the higher the tax paid, the more likely that the people are richer, and therfore crime could be higher.

```{r}
summary(crime_data_corr2$taxpc)
cor(crime_data_corr2$crmrte,crime_data_corr2$taxpc)
```
...
```{r}
hist(crime_data_corr2$taxpc, breaks = 50)
```

Examining the relationship between pctymle and pctmin80 with crmrte:

```{r}
scatterplotMatrix(~ log(crime_data_corr2$crmrte) + crime_data_corr2$pctymle +  crime_data_corr2$pctmin80)
```

The crime rate is higher in places with more % of young males. This seems somewhat likely.  The crime rate is higher gnerally when minority % is higher. However, both variables seem to have non-ideal distributions.

Looking at the correlation between the variables:
```{r}
summary(crime_data_corr2$pctymle)
cor(crime_data_corr2$crmrte,crime_data_corr2$pctymle)
summary(crime_data_corr2$pctmin80)
cor(crime_data_corr2$crmrte,crime_data_corr2$pctmin80)
```
The correlation is not very high though.

Finally looking at avgsen,
```{r}
summary(crime_data_corr2$avgsen)
cor(crime_data_corr2$crmrte,crime_data_corr2$avgsen )
```
...
```{r}
scatterplotMatrix(~ log(crime_data_corr2$crmrte) + crime_data_corr2$avgsen)
```
