...
As a result, we will apply a log() transformation to the variable, which will address both issues. This transformation will change our interpretation as well, since the result will need to be interrpeted as a percentage change for Crime Rate.. 
...
```{R, results='asis'}
model1 = lm(crime_data_clean$crmrte ~ 
...
Looking at a second model that has a log transformation of the crmrte variable:

## Model1_log: Log of crmrte 
```{R, results='asis'}
model1_logy = lm(log(crime_data_clean$crmrte) ~ 
              crime_data_clean$prbarr + 
              crime_data_clean$prbconv_fix + 
              crime_data_clean$prbpris + 
              crime_data_clean$avgsen)
...
The Q-Q plot got better for sure though it is difficult to see if any of the other ones are better. We can use Stargazer to figure out.


## Model3: With all variables

```{r}
model3 = lm(crime_data_clean$crmrte ~ crime_data_clean$prbarr + crime_data_clean$prbconv_fix + crime_data_clean$prbpris + crime_data_clean$avgsen + crime_data_clean$polpc + crime_data_clean$density + crime_data_clean$taxpc + crime_data_clean$pctmin80 + crime_data_clean$pctymle)
```

```{r}
model3_logy = lm(log(crime_data_clean$crmrte) ~ crime_data_clean$prbarr + crime_data_clean$prbconv_fix + crime_data_clean$prbpris + crime_data_clean$avgsen + crime_data_clean$polpc + crime_data_clean$density + crime_data_clean$taxpc + crime_data_clean$pctmin80 + crime_data_clean$pctymle)
```


```{r}
stargazer(model1_logy, model3_logy, star.cutoffs = c(0.05,0.01, 0.001), type = "text", float=FALSE)
```

```{R}
AIC(model1_logy, model3_logy)
```
...
```{r}
model2 = lm(crime_data_clean$crmrte ~ crime_data_clean$prbarr +  crime_data_clean$density +  log(crime_data_clean$polpc) + crime_data_clean$pctmin80)
model2_logy = lm(log(crime_data_clean$crmrte) ~ crime_data_clean$prbarr + crime_data_clean$density +  log(crime_data_clean$polpc) + 
                   crime_data_clean$pctmin80)
```

Let's plot the model2_logy and look at the violations:
```{r}
plot(model2_logy)
```
...
```{r}
#Since models are heteroskedastic, need to use heteroskedastic-robust SEs
se.model1_logy = sqrt(diag(vcovHC(model1_logy)))
se.model2_logy = sqrt(diag(vcovHC(model2_logy)))
se.model3_logy = sqrt(diag(vcovHC(model3_logy)))
stargazer(model1_logy, model2_logy, model3_logy, 
          se = list(se.model1_logy, se.model2_logy, se.model3_logy),
...
```{r}
AIC(model1_logy, model2_logy, model3_logy)
