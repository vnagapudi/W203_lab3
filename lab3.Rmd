...
Now there are only 79 observerations which is less than 100. So we do need to be careful our assumptions around CLM for coefficients being normal.
...
As we can see, the log(crimrte) seems to be negatively correlated with prbarr and prbconv_fix which seems to be intuitive. There is perhaps a positive correlation to prbpris, which is not very intuitive, but the direction of the correlation is not clear from the dataset and therefore we exclude this from our key variable set.
