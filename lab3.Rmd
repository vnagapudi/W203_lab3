...
2. ue to the presence of a random back-tick character in the now removed "NA" rows at the end of the dataset, the Variable prbconv was interpreted as a factor of levels - we can convert it back to numeric data with no loss.
```{r}
...
We note the cases with particularly high correlation with crmrte and examine them below.
...
```
```{R}
ggplot(data=crime_data_clean, aes(log(crmrte))) + 
  geom_histogram(aes(y =..count..), bins=50) + geom_density(aes(y = ..scaled..))
  labs(title="...................", x=".....", y=".......")
```
